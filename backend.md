...
| GET    | `/activity` | `verifySuperAdmin` | `getNotificationActivity` (`broadcast.Controller.js:134` `99c1870`): unified Auto-tab feed of system-generated notifications. `?source=all|offer|ad|event`, `?event=<key>`, `?status=<status>`, `?page=1` (limit 50). Broadcast rows (`source IN ('offer','ad')`, `event` mapped to `offer_announcement`/`ad_announcement`, `audienceLabelForRow`, `channels`, `sent/failed/target_count`) merged with event rows (`notifications WHERE broadcast_id IS NULL`, channels from `notification_dispatch_log` incl. `in_app`, status `failed|queued|sent|in_app_only`) → sorted by `created_at DESC`, paginated → `{activity, total, page, limit}` |
