...
Unified in-app + email notification center backed by `notifications`/`notification_dispatch_log`/`device_tokens`/`customer_settings` (`docs/db_setup.sql` — same migration as `migrations/migration_notifications.sql:1`, idempotent). Events: `booking_confirmed`, `booking_cancelled`, `refund_initiated`, `refund_settled`, `refund_failed` (`eb8920c`), `show_cancelled`, `show_reminder`, `daily_report`, `security_alert`, `team_role_changed` (`eb8920c`), `team_removed` (`eb8920c`), `team_invite_accepted` (`eb8920c`) — title/body via `buildNotificationContent(event,data)` (`services/notification/templates.js:5` → `eb8920c` adds `refund_failed`, `team_role_changed`, `team_removed`, `team_invite_accepted` cases).
