...
erDiagram
    ...
    admin_broadcasts {
        ...
        text body
        text image_url
        text audience_type "all_customers|all_admins|custom|hall_customers CHECK (99c1870)"
        uuid[] recipient_customer_ids "DEFAULT '{}'"
        uuid[] recipient_admin_ids "DEFAULT '{}'"
        jsonb device_token_filter "DEFAULT '{}' (c227618)"
        text[] channels "DEFAULT '{push}' - subset of push|email (99c1870)"
        text source "manual|offer|ad CHECK, DEFAULT 'manual' (99c1870)"
        uuid origin_id "offer/ad that spawned it, deliberately not a FK (99c1870)"
        uuid cinema_hall_id FK "REFERENCES cinema_hall ON DELETE SET NULL (99c1870)"
        int target_count
        int sent_count
        int failed_count
...
| POST   | `/create`        | SuperAdmin    | Create a new ad — accepts optional `notify: {enabled, channels, title, body}` and announces to `all_customers` on create (`99c1870`); response includes `announced: true/false` |
| PUT    | `/update/:id`    | SuperAdmin    | Update an existing ad                            |
| DELETE | `/delete/:id`    | SuperAdmin    | Delete an ad (cascades click records)            |
| POST   | `/:id/announce`  | SuperAdmin    | Announce an existing ad (`99c1870` `announceAdById`): validates the ad exists, `announceAd` → `admin_broadcasts` row (`source='ad'`, `origin_id`), `201 {broadcast}` |
...
| POST   | `/create`            | `offers.create` | Create offer. `scope=global` requires SuperAdmin (`403 Only Super Admin can create global offers.`); `scope=hall` for non-SuperAdmin requires `cinema_hall_id` belonging to `resolveOrgId` org (`403 You can only create offers for your own cinema hall.`) (`controllers/offers.Controller.js:221`). Accepts optional `notify: {enabled, channels, title, body}` and announces on create (`99c1870` `announceOffer`) — `all_customers` for global scope, `hall_customers` for hall scope; response includes `announced: true/false` |
...
| POST   | `/:id/announce`      | `offers.update` | Re-announce an existing offer (`99c1870` `announceOfferById`): owner/superAdmin + hall-org guard mirror update; `announceOffer` → `admin_broadcasts` row (`source='offer'`, `origin_id`), audit log `offers.announce`; `201 {broadcast}` |
...
Unified in-app + email notification center backed by `notifications`/`notification_dispatch_log`/`device_tokens`/`customer_settings` (`docs/db_setup.sql` — same migration as `migrations/migration_notifications.sql:1`, idempotent). Events: `booking_confirmed`, `booking_cancelled`, `refund_initiated`, `refund_settled`, `refund_failed` (`eb8920c`), `show_cancelled`, `show_reminder` (`b434bee` scheduled), `daily_report`, `security_alert`, `team_role_changed` (`eb8920c`), `team_removed` (`eb8920c`), `team_invite_accepted` (`eb8920c`) — title/body via `buildNotificationContent(event,data)` (`services/notification/templates.js:5` → `eb8920c` adds `refund_failed`, `team_role_changed`, `team_removed`, `team_invite_accepted` cases).
...
| GET    | `/`      | `identifyRecipient` (customer `cusAccessToken`/`Bearer` or admin `accessToken`, `localhost` dual-cookie via `Origin===ADMIN_FRONTEND_URL`) | `listNotifications` (`controllers/notifications.Controller.js:7` → `b434bee` ` controllers/notifications.Controller.js:16`): `page`/`limit` 1-100 default 20, `WHERE customer_id|admin_id AND (scheduled_for IS NULL OR scheduled_for <= now())`, `ORDER BY created_at DESC` → `{notifications:[{id,event,title,body,data,booking_id,show_id,refund_id,read_at,created_at}]}`, `page`, `limit` |
| GET    | `/unread-count` | `identifyRecipient` | `getUnreadCount` (`:32` → `b434bee` `:34`): `COUNT(*) WHERE … read_at IS NULL AND (scheduled_for IS NULL OR scheduled_for <= now())` → `{count}` |
...
| POST   | `/device-token` | `identifyRecipient` | `registerDeviceToken` (`controllers/notifications.Controller.js:9` `e121b87`): body `{token, platform: web|android|ios}`, `INSERT INTO device_tokens (customer_id|admin_id, token, platform) VALUES … ON CONFLICT (token) DO UPDATE SET platform, last_seen_at=now()` — `UNIQUE token`, `platform CHECK web|android|ios`, one-recipient CHECK |
| DELETE | `/device-token` | `identifyRecipient` | `unregisterDeviceToken` (`:9` `e121b87`): body `{token}`, `DELETE FROM device_tokens WHERE token=$1 AND recipient = $2` — idempotent |

**FCM push (`e121b87` `services/notification/firebaseAdmin.js:1` / `services/notification/channels/push.js:1` / `services/notification/dispatch.Controller.js:64` / `package.json:29` `firebase-admin@^14.3.0`):** `firebaseAdmin.js:1` `initializeApp(cert({projectId: FIREBASE_PROJECT_ID, clientEmail: FIREBASE_CLIENT_EMAIL, privateKey: replace(\\n,\n)}))` or `getApps()[0]`, `messaging=getMessaging(app)` (`.env` `FIREBASE_PRIVATE_KEY` stored escaped; `.gitignore` allows `service-account*.json` exception). `push.js:9` `sendPushForNotification(recipient,notification)` → `SELECT token FROM device_tokens WHERE recipient`, throws `no registered devices` if none, `messaging.sendEachForMulticast({tokens, notification:{title,body}, data:{event,notificationId}})`, deletes `registration-token-not-registered` dead tokens `DELETE ... id=ANY`, throws if `successCount===0`, returns `join(",")` for `dispatch_log.target`. `dispatch.Controller.js:64` now branches `email` → `sendEmailForNotification` / `push` → `sendPushForNotification({type,id},notification)` / else `not implemented`; `qstashReceiver.verify({signature, body, url: API_BASE_URL/api/notifications/dispatch})` — raw `Buffer` from `express.raw` (`server.js:35` `RAW_BODY_PATHS` includes `/api/notifications/dispatch`).

**New events & trigger sites (`eb8920c` / `3965271`):** `DEFAULT_EVENT_PREFERENCES` (`defaultPreferences.js:12`) and `USER_DEFAULTS.notifications` (`3965271` in `cinema-hall-admin/src/lib/settings/settingsDefaults.js:91`) add `refund_failed: {email:true push:true}`, `team_role_changed: {email:false push:true}`, `team_removed: {email:true push:true}`, `team_invite_accepted: {email:false push:true}` alongside existing 8; `templates.js:46` adds 4 `buildNotificationContent` cases (refund_failed with `reason`, team_role_changed with `roleLabel/status`, team_removed, team_invite_accepted with `newAdminName`). Notify sites (all `try/catch logger.error non-fatal` after COMMIT): `auth.Controller.js:16` `loginCinemaAdmin` lockout (423) + `resetPassword`/`changePassword` + `acceptInvite` (resolves `inviterAdminId` via `member.invited_by || organizations.owner_id`, `team.service.js:311` now returns `newAdminId/newAdminName/orgId/inviterAdminId` via `SELECT a.name,a.email` + `RETURNING org_id,invited_by`), `payment.Controller.js:543` `refund.failed` webhook (`RETURNING` + `owner_id` lookup), `shows.Controller.js:592` `cancelShow`/`bulkCancelShows` per-booking `refund_failed` branch (`JOIN organizations`), `team.Controller.js:7` `updateMember` → `team_role_changed` (fetches `roles.label`) and `removeMember` → `team_removed`.

**Dispatch pipeline (`services/notification/index.js:19` `notify(event,recipient,data)`, `scheduleShowReminder`, `cancelShowReminder`):** called *after* triggering transaction `COMMIT` (so notify failure never 500s): `insertInAppNotification` (`channels/inApp.js:8` `b434bee` — `INSERT INTO notifications (org_id,customer_id,admin_id,event,title,body,data,booking_id,show_id,refund_id,scheduled_for) VALUES … RETURNING *` with `buildNotificationContent`, param `scheduledFor` `null` normally else `reminderAt` for `scheduleShowReminder`), `resolveEnabledChannels` (`preferences.js:12`: intersects org `organization_settings` `notifications` master switches + recipient per-event `customer_settings`/`user_settings` with `DEFAULT_EVENT_PREFERENCES` defaults), per-channel `INSERT notification_dispatch_log … queued` + `publishDispatch` (`qstashClient.js:30` `publishJSON {url: API_BASE_URL/api/notifications/dispatch, body:{notificationId,event,recipientType,recipientId,channel}, notBefore?, retries:3}` → stored `qstash_message_id`). `show_reminder` (`index.js:89` `b434bee`) scheduled at `showDateTime -60m` (`notBefore` epoch + `scheduled_for=reminderAt` so in-app row hidden until due via `(scheduled_for IS NULL OR scheduled_for <= now())`), skipped if `reminderAt <= now`; cancellation queries `d.queued` reminders for `booking_id` and `cancelScheduledMessage` (`qstashClient.messages.delete`) → `skipped`; `listNotifications` (`controllers/notifications.Controller.js:16` `b434bee`) and `getUnreadCount` (`:34`) filter `scheduled_for` similarly.
...
| Method | Endpoint | Auth | Description |
| ------ | -------- | ---- | ----------- |
| POST   | `/broadcast` | `verifySuperAdmin` | `createBroadcast` (`broadcast.Controller.js:8`): body `{title*, body, imageUrl, audienceType: all_customers|all_admins|custom|hall_customers*, customerIds?, adminIds?, cinemaHallId?, channels?, deviceTokenFilter?, scheduledFor?}` validates title, audienceType in `AUDIENCE_TYPES`, custom needs `customerIds||adminIds`, `hall_customers` needs `cinemaHallId`, `channels` must be a subset of `push|email` (default `['push']` — in-app always fires), parses `scheduledFor` `Date`, `isFutureSend=scheduledDate>Date.now()`, `resolveAudience` → recipients, `INSERT INTO admin_broadcasts (created_by,title,body,image_url,audience_type,recipient_customer_ids,recipient_admin_ids,device_token_filter,target_count=recipients.length,scheduled_for=isFutureSend?date:null,channels,source='manual',cinema_hall_id) RETURNING *`; if `isFutureSend` → `scheduleBroadcastFor(broadcast,recipients,scheduledDate)` → `201 {broadcast:{...status:'scheduled'}}` else `sendBroadcastNow` → `201 {broadcast:{...status:'sent',sent_count,failed_count}}`; errors `400` title/audience/custom/hall/channels/scheduledFor invalid, `500` |
| GET    | `/broadcast` | `verifySuperAdmin` | `listBroadcasts` (`:64`): `?source=manual|offer|ad|all` (default `manual` so the Manual tab excludes auto announcement rows) → `SELECT b.*, a.name AS created_by_name FROM admin_broadcasts LEFT JOIN cinema_admin_user WHERE $1='all' OR b.source=$1 ORDER BY created_at DESC LIMIT 100` → `{broadcasts:[{id,created_by,created_by_name,title,body,image_url,audience_type,recipient_customer_ids,recipient_admin_ids,channels,source,origin_id,cinema_hall_id,target_count,sent_count,failed_count,status,scheduled_for,sent_at,created_at}]}` |
| GET    | `/activity` | `verifySuperAdmin` | `getNotificationActivity` (`broadcast.Controller.js:134` `99c1870`): unified Auto-tab feed of system-generated notifications. `?source=all|offer|ad|event`, `?event=<key>`, `?status=<status>`, `?page=1` (limit 50). Broadcast rows (`source IN ('offer','ad')`, `event` mapped to `offer_announcement`/`ad_announcement`, `audienceLabelForRow`, `channels ['in_app', ...channels]` since `34698c7`, `sent/failed/target_count`) merged with event rows (`notifications WHERE broadcast_id IS NULL`, channels from `notification_dispatch_log` incl. `in_app`, status `failed|queued|sent|in_app_only`) → sorted by `created_at DESC`, paginated → `{activity, total, page, limit}` |
| GET    | `/device-tokens` | `verifySuperAdmin` | `getDeviceTokensForPicker` (`:6` `c227618`): `?type=customer|admin&id=<uuid>` (else `400 type (customer|admin) and id are required`) → `SELECT id, platform, last_seen_at, created_at FROM device_tokens WHERE customer_id|admin_id ORDER BY last_seen_at DESC` → `{tokens}` |
| DELETE | `/broadcast/:id` | `verifySuperAdmin` | `deleteBroadcast` (`broadcast.Controller.js:144` `af3a008`): `services/notification/broadcast.js:149` cancels pending `notification_dispatch_log` `queued` `qstash_message_id` via `cancelScheduledMessage` (marks `skipped`, warns non-fatal), `DELETE FROM notifications WHERE broadcast_id=$1` (removes from all recipient in-app feeds), `DELETE FROM admin_broadcasts WHERE id=$1` (keeps dispatch_log as audit trail, FK `ON DELETE SET NULL`); `200 {deleted:true}` / `404 Broadcast not found` |
...
**Multi-channel + announcements migration (`migration_notification_channels.sql:1` `99c1870`):** adds `channels TEXT[] NOT NULL DEFAULT '{push}'` (only push/email stored — in-app is implicit), `source TEXT NOT NULL DEFAULT 'manual'` + `CHECK (source IN ('manual','offer','ad'))`, `origin_id UUID` (deliberately **not** a FK — an announcement outlives the offer/ad that spawned it), `cinema_hall_id UUID REFERENCES cinema_hall ON DELETE SET NULL`; widens the audience CHECK to admit `hall_customers`; indexes `idx_admin_broadcasts_source (source, created_at DESC)`, `idx_admin_broadcasts_origin (origin_id) WHERE origin_id IS NOT NULL`, and `idx_notifications_event_created (event, created_at DESC)` (powers the Auto tab's event feed).

**Service `services/notification/broadcast.js:1` (`622af22` / `99c1870`):** `resolveAudience({audienceType,customerIds,adminIds,deviceTokenFilter,cinemaHallId})` → each recipient now carries `{type, id, email, name, deviceTokenIds}` (email/name so `sendBroadcastNow` can send synchronously without the QStash re-fetch): `all_customers` `SELECT id,email,name FROM customers` / `all_admins` `SELECT id,email,name FROM cinema_admin_user WHERE role='admin'` / `hall_customers` (new, `99c1870`) `SELECT DISTINCT customers` joined `bookings→shows→screens WHERE screens.cinema_hall_id=$1 AND bookings.customer_id IS NOT NULL` (customers who booked at a hall — used for hall-scoped offers) / `custom` fetches name/email for the requested ids + applies `deviceTokenFilter`. `insertDispatchLogRow({notificationId,broadcastId,recipient,channel,status,target,error})` inserts `notification_dispatch_log (notification_id,broadcast_id,customer_id,admin_id,event='admin_broadcast',channel,status,target,error,attempted_at=now())`. `sendBroadcastNow(broadcast,recipients,extraData)` synchronous (no QStash, works localhost) — per recipient `insertInAppNotification(event='admin_broadcast', data:{title,body,imageUrl,...extraData})` → `UPDATE notifications SET broadcast_id` + `insertDispatchLogRow in_app sent`, then `resolveEnabledChannelsForBroadcast` intersected with `broadcast.channels` (the admin's pick is a **ceiling**, not an override) and sends each channel via `sendExternalChannel` (`push`→`sendPushForNotification({tokenIds})`, `email`→`sendEmailForNotification`), logging per-channel `sent`/`failed`; a recipient counts `sent` when in-app fired and every *requested* channel that was allowed succeeded (or none were allowed), else `failed`. Final `UPDATE admin_broadcasts SET status='sent', sent_count, failed_count, sent_at=now()`. `scheduleBroadcastFor(broadcast,recipients,scheduledFor,extraData)` computes `notBefore=epochSeconds`, per recipient `insertInAppNotification(..., scheduledFor)` + per-channel `INSERT dispatch_log queued` → `publishDispatch({notificationId,event='admin_broadcast',recipientType,recipientId,channel,notBefore})` → `UPDATE qstash_message_id` else `failed`, final `UPDATE admin_broadcasts SET status='scheduled'`.

**Device-token filtering (`c227618` in `migrations/migration_admin_broadcasts_device_filter.sql:1` / `controllers/broadcast.Controller.js:6` / `services/notification/broadcast.js:7` / `services/notification/channels/push.js:9` / `services/notification/dispatch.Controller.js:40` / `services/notification/preferences.js:25` / `services/notification/qstashClient.js:34` / `routes/notifications.routes.js:23`):** `admin_broadcasts.device_token_filter JSONB NOT NULL DEFAULT '{}'` (idempotent `ALTER TABLE`), shape `{"customer:<id>": ["<device_tokens.id>",...], "admin:<id>":[...]}` — key present only when narrowed, absent = all devices; `GET /api/notifications/device-tokens?type=customer|admin&id=<uuid>` (`verifySuperAdmin`, `broadcast.Controller.js:6` `getDeviceTokensForPicker` `SELECT id,platform,last_seen_at,created_at FROM device_tokens WHERE customer_id|admin_id ORDER BY last_seen_at DESC` → `{tokens}`); `resolveAudience` returns `{type,id,deviceTokenIds}` (`deviceTokenFilter[`customer:${id}`]||null`); `resolveEnabledChannelsForBroadcast` (`preferences.js:25` — skips org switch, platform-wide); `sendPushForNotification(recipient,notification,{tokenIds})` (`push.js:9` — `WHERE id=ANY($1::uuid[]) AND recipient=$2` when narrowed); `publishDispatch({...,tokenIds})` (`qstashClient.js:34`) + `dispatch.Controller.js:40` forwards `tokenIds`; `routes/notifications.routes.js:23` `GET /device-tokens` `verifySuperAdmin`.

**Delete (`af3a008` `controllers/broadcast.Controller.js:144` / `services/notification/broadcast.js:149` / `routes/notifications.routes.js:17`):** `deleteBroadcast(broadcastId)` — finds pending `notification_dispatch_log` (`WHERE broadcast_id=$1 AND status='queued' AND qstash_message_id IS NOT NULL`), `cancelScheduledMessage` each + `skipped` (warn non-fatal), `DELETE FROM notifications WHERE broadcast_id=$1`, `DELETE FROM admin_broadcasts WHERE id=$1` (returns `rowCount>0`); `dispatch_log` rows kept (broadcast_id → NULL via FK); route `router.delete('/broadcast/:id', verifySuperAdmin, deleteBroadcast)`.
...
**Defaults/templates (`defaultPreferences.js:18` / `templates.js:69` `622af22` / `99c1870`):** `DEFAULT_EVENT_PREFERENCES` adds `admin_broadcast: {email: true, push: true}` (`99c1870` flipped email to `true` — the admin's channel pick is the ceiling, so this just means "hasn't opted out"); `templates.js:69` `admin_broadcast` returns `{title: data.title||'Notification', body: data.body||''}` (super admin-composed, not templated).

**Offer/Ad announcements (`99c1870` in `services/notification/announce.js:1` / `controllers/offers.Controller.js:467` / `controllers/ads.Controller.js:96`):** `announceOffer(offer, opts)` / `announceAd(ad, opts)` bridge Offers/Ads into the broadcast pipeline — both insert an `admin_broadcasts` row with `source: 'offer'|'ad'` + `origin_id` and run `runAnnouncement` (reusing `resolveAudience` + `sendBroadcastNow`/`scheduleBroadcastFor`). `announceOffer` defaults copy from the offer (`defaultOfferCopy` — `title`/`body` with `formatDiscount`, validity, min amount), audience `hall_customers` when `scope='hall'` else `all_customers`; `announceAd` defaults from the ad (`defaultAdCopy`) + `image_url` and a `ctaUrl`/`ctaLabel` (`click_url` → "View details" since `a839052`) as `extraData`. Wired as `POST /api/offers/:id/announce` (`offers.update`, owner/superAdmin + hall-org guard, audit log `offers.announce`) and `POST /api/ads/:id/announce` (`verifySuperAdmin`); `createOffer`/`createAd` also accept an optional `notify: {enabled, channels, title, body}` and announce on create, returning `announced: true/false` — announcement failures are always swallowed (`logger.error`) so a dead FCM token/SMTP hiccup never fails offer/ad creation. Email for broadcasts uses `sendBroadcastEmail` (`mail/emails.js:361` `99c1870`) with the generic `BROADCAST_TEMPLATE` (`mail/emailTemplate.js:834` — `{name}`, `{title}`, `{message}`, pre-rendered `{imageBlock}`/`{ctaBlock}`); wired via `channels/email.js:54` `admin_broadcast → sendBroadcastEmail`.
