...
- Page `BroadcastNotifications.jsx:1` (`9368b5e`): header `Send Notifications` + `New Notification` `Plus` button; history `loadBroadcasts` → `broadcastAPI.list()` `GET /api/notifications/broadcast` (`verifySuperAdmin`, `LIMIT 100` with `created_by_name`), table columns Title (with `image_url` thumb `size-10 rounded-md` + `line-clamp-1` body), Audience (`audienceLabel` `all_customers`/`all_admins`/`custom n people` from `recipient_customer_ids/admin_ids` lengths), Status (`statusBadge` `sent` green `bg-green-500/20` / `scheduled` blue `bg-blue-500/20` else `outline`), Delivered/Failed `sent_count/failed_count of target_count` (green/red), When (`formatDateTime en-IN` `scheduled_for` if scheduled else `sent_at || created_at`), By (`created_by_name`); row `hover:bg-muted/30 cursor-pointer` opens detail `Dialog` (`max-w-2xl`) with `dispatch_log` table (`notification_dispatch_log` joined `customers`/`cinema_admin_user` Coalesce `recipient_name/email`, `channel`, `status` `sent/delivered` green `CheckCircle2` / `failed` red `XCircle` / else muted) and summary `sent delivered, failed of target`.
- Compose `Sheet` (`sm:max-w-xl`, `overlayClassName backdrop-blur-sm`, form `broadcast-form`): fields Title* (required input), Message* (`Textarea` 3 rows), Image (`input file image/*` → `uploadImageToCloudinary` `result.url` → `imageUrl` preview `h-28 object-cover` + `uploading`), Audience* (`AUDIENCE_CHOICES` `all_customers Users`, `all_admins Building2`, `custom Plus` — grid 3, `border-primary bg-primary/10` when active), People search (custom only): debounced 400ms `searchPeople` `customersAPI.getAll({search,limit:6})` + `adminsAPI.getAll({search,limit:6})` → combined `peopleResults`, dropdown with `X` remove pills `bg-primary/10 border-primary/20 rounded-full`, Delivery* (`grid 2` `Zap Send Now` vs `Clock Schedule` — `delivery: now|schedule`, `scheduledFor` `datetime-local` required when schedule), submit `handleSubmit` validates custom needs `people.length>0` and schedule needs `scheduledFor`, payload `{title, body, imageUrl||undefined, audienceType, customerIds: people.filter(customer).map, adminIds, scheduledFor: delivery==='schedule'? new Date(...).toISOString(): undefined}` → `broadcastAPI.create` (`POST /api/notifications/broadcast`), toast `Sent — {sent} delivered, {failed} failed` vs `Scheduled for {formatDateTime}`; `formOpen`/`formLoading`/`uploading`.
- Service `broadcastAPI` (`services/api.js:1134` `9368b5e`): `list()` `GET /api/notifications/broadcast`, `get(id)` `GET /broadcast/:id`, `create(data)` `POST /broadcast` all `credentials:"include"` (Super Admin).
