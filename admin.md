...
| **Actions** | Announce (`Megaphone` ghost icon, `5e69dd9` — opens the Announce offer `Sheet` with a `NotifyBlock`) / Edit (pencil) / Delete (trash) gated to `isSuperAdmin \|\| offer.created_by === user.id` (`src/pages/OffersManagement.jsx:384`, `a79e555`); otherwise `—` muted |
...
**Announce (`5e69dd9`):** the **create-only** form (`/offers/new`, `!isEdit`) adds a third **Announce** card with a shared `NotifyBlock` (`src/components/notifications/NotifyBlock.jsx`): a "Notify users" toggle (Megaphone + `Switch`) revealing Channel picker (`ChannelPicker`), optional Title / Message (`Textarea`). Audience label is dynamic — `Customers who booked at <hall>` when `scope="hall"`, else `All customers`. On submit `offersAPI.create({...payload, notify: notifyPayload(notify)})`; the toast reads `Offer created and customers notified.` when the API returned `announced: true`. Existing offers are re-announced from the list page: the row's `Megaphone` button opens an **Announce offer** `Sheet` (`sm:max-w-md`, `NotifyBlock` pre-enabled) → `offersAPI.announce(id, notifyPayload(form))` → `POST /api/offers/:id/announce`, toast `Sent — {sent} delivered, {failed} failed` vs `Scheduled for {scheduled_for}`.
...
- Edit / Announce (`Megaphone`, `5e69dd9`) / Delete buttons
...
| **Actions** | Edit, Announce (`Megaphone`), and Delete buttons |
...
| Notify users | Block    | No       | **Create-only** (`!editingAd`, `5e69dd9`) — shared `NotifyBlock` below a `border-t` separator: "Notify users" toggle + `ChannelPicker` (push/email) + optional Title/Message; `audienceLabel="All customers"`; submit sends `adsAPI.create({...formData, notify: notifyPayload(notify)})` → toast `Ad created and customers notified` when `announced: true` |

**Announce ad (`5e69dd9`):** the row/card `Megaphone` button opens an **Announce ad** `Sheet` (`side="right"`, `sm:max-w-md`, `NotifyBlock` pre-enabled, audience "All customers") → `adsAPI.announce(ad.id, notifyPayload(form))` → `POST /api/ads/:id/announce`, toast `Sent — {sent} delivered, {failed} failed` vs `Scheduled for {scheduled_for}`.
...
**Component**: `PaymentOrders.jsx`
**Sidebar**: Operations → Payment Orders (`CreditCard` icon, after Refunds)
...
- Route `/notifications` (`App.jsx:46`, `ProtectedRoute`) — paginated in-app feed `PAGE_SIZE=20` (`Notifications.jsx:7`): `load(targetPage)` → `notificationAPI.list(targetPage,PAGE_SIZE)` merging `setNotifications(prev=>page===1?list:[...prev,list])`, `hasMore = list.length===PAGE_SIZE`, `loading`; `useEffect load(1)` on mount; optimistic `handleItemClick` (`read_at=now` + `markAsRead`) and `handleMarkAllRead` (`all read_at || now` + `markAllRead`); header now has `Enable push` `BellRing` `outline` + `Mark all read` (`Notifications.jsx:13` `35908f4`) — `pushEnabled = Notification.permission==="granted"` state, `pushBusy`, `handleEnablePush` (`requestPushToken()` → `notificationAPI.registerDeviceToken(token,"web")` → `setPushEnabled(true)`), empty `Bell No notifications yet`; list `button read_at? bg-primary dot` with `title` (`!read_at font-medium`) + `body` + `formatDistanceToNow`; `Load more` `Button` `loading? Loading…`.
- Service `notificationAPI` (`src/services/api.js:1056` → `35908f4` `api.js:1101`): `list(page,limit)` `GET /api/notifications?page&limit`, `getUnreadCount` `GET /unread-count`, `markAsRead(id)` `PATCH /:id/read`, `markAllRead` `PATCH /read-all`, `getPreferences` `GET /preferences`, `updatePreferences(patch)` `PATCH /preferences {patch}`, `registerDeviceToken(token,platform="web")` `POST /api/notifications/device-token {token,platform}`, `unregisterDeviceToken(token)` `DELETE /api/notifications/device-token {token}` — all `credentials:"include"` and `throw await response.json()`.
- Push via Firebase (`src/lib/firebase.js:1` + `public/firebase-messaging-sw.js:1`, `35908f4` → `5e69dd9` for image support): `firebase` `initializeApp` with `VITE_FIREBASE_*` (`firebase.js:8`), `requestPushToken()` (`firebase.js:20`: `isSupported()` → `Notification.requestPermission()` → `navigator.serviceWorker.register("/firebase-messaging-sw.js")` → `getToken(messaging,{vapidKey: VITE_FIREBASE_VAPID_KEY, serviceWorkerRegistration})`), `onForegroundMessage(callback)` via `onMessage`; SW duplicates public config (`importScripts 10.14.1/firebase-app-compat.js/messaging-compat.js`, `cinemax-644cf` `apiKey`, `messagingSenderId 212799048998`, `onBackgroundMessage` → `self.registration.showNotification(title||'CineMax Admin', {body, icon:'/cinemax_logo.png'})`); since `5e69dd9` the SW also reads `image` from `payload.notification` (the admin SDK's `imageUrl` renames to `image` client-side) and passes it to `showNotification`; package `firebase` added.

**Broadcast Notifications** (`src/pages/BroadcastNotifications.jsx:1` / `src/services/api.js:1134` / `src/App.jsx:47` / `src/components/AppSidebar.jsx:9`, `9368b5e` / `622af22` for API)

- Route `/notifications/broadcast` (`App.jsx:47` `AdminProtectedRoute requireSuperAdmin={true}` at `App.jsx:153`) — Super Admin-only history + compose. `AppSidebar.jsx:9` adds `managementItems` `Notifications` `Send` icon `superAdminOnly:true` (alongside Hall Admins) under Management.
- Page `BroadcastNotifications.jsx:1` (`9368b5e` / `5e69dd9`): header `Send Notifications` + `New Notification` `Plus` button. Since `5e69dd9` the page is split into **Manual / Auto** tabs (`Tabs`, `Send`/`Radio` icons): **Manual** is the composer history — `loadBroadcasts` → `broadcastAPI.list({ source: 'manual' })` `GET /api/notifications/broadcast?source=manual` (`verifySuperAdmin`, `LIMIT 100` with `created_by_name`; the `source` filter excludes auto rows an Offer/Ad announcement generates), table columns Title (with `image_url` thumb `size-10 rounded-md` + `line-clamp-1` body), Audience (`audienceLabel` `all_customers`/`all_admins`/`custom n people` from `recipient_customer_ids/admin_ids` lengths), Status (`statusBadge` `sent` green `bg-green-500/20` / `scheduled` blue `bg-blue-500/20` else `outline`), Delivered/Failed `sent_count/failed_count of target_count` (green/red), When (`formatDateTime en-IN` `scheduled_for` if scheduled else `sent_at || created_at`), By (`created_by_name`); row `hover:bg-muted/30 cursor-pointer` opens detail `Dialog` (`max-w-2xl`) with `dispatch_log` table and summary `sent delivered, failed of target`. **Auto** renders `NotificationActivityTab` (see below).
- Compose `Sheet` (`sm:max-w-xl`, `overlayClassName backdrop-blur-sm`, form `broadcast-form`): fields Title* (required input), Message* (`Textarea` 3 rows), Image (`input file image/*` → `uploadImageToCloudinary` `result.url` → `imageUrl` preview `h-28 object-cover` + `uploading`), **Channels** (`ChannelPicker` since `5e69dd9` — grid of Push/Email toggles; in-app is always sent), Audience* (`AUDIENCE_CHOICES` `all_customers Users`, `all_admins Building2`, `custom Plus` — grid 3, `border-primary bg-primary/10` when active), People search (custom only): debounced 400ms `searchPeople` `customersAPI.getAll({search,limit:6})` + `adminsAPI.getAll({search,limit:6})` → combined `peopleResults`, dropdown with `X` remove pills `bg-primary/10 border-primary/20 rounded-full`, Delivery* (`grid 2` `Zap Send Now` vs `Clock Schedule` — `delivery: now|schedule`, `scheduledFor` `datetime-local` required when schedule), submit `handleSubmit` validates custom needs `people.length>0` and schedule needs `scheduledFor`, payload `{title, body, imageUrl||undefined, audienceType, customerIds: people.filter(customer).map, adminIds, deviceTokenFilter, channels: formData.channels, scheduledFor: delivery==='schedule'? new Date(...).toISOString(): undefined}` → `broadcastAPI.create` (`POST /api/notifications/broadcast`), toast `Sent — {sent} delivered, {failed} failed` vs `Scheduled for {formatDateTime}`; `formOpen`/`formLoading`/`uploading`.
- **NotificationActivityTab** (`src/components/notifications/NotificationActivityTab.jsx:1`, `5e69dd9`): the Auto tab's unified feed of system-generated notifications — source filter (`All sources / Offers / Ads / Events`) + event filter (`EVENT_OPTIONS`: `booking_confirmed`, `booking_cancelled`, `refund_initiated/settled/failed`, `show_cancelled`, `show_reminder`, `security_alert`, `team_role_changed`, `team_removed`, `team_invite_accepted`) → `broadcastAPI.activity({source,event})` `GET /api/notifications/activity`. Table columns Notification (kind icon — `Tag` violet for offer, `Megaphone` primary for ad, `Bell` sky for event — + title/body), Trigger (`Badge` from `event`), Recipient(s) (`recipient_label`), Channels (`row.channels` joined), Status (`statusBadge` sent/failed/queued/scheduled/`in_app_only`), When (`formatDateTime`); row click opens a detail `Dialog` — broadcast rows re-fetch `broadcastAPI.get(origin_id)` for the dispatch-log table, event rows show inline fields; empty state `Radio` icon + "No system-triggered notifications yet."
- **ChannelPicker** (`src/components/notifications/ChannelPicker.jsx:1`, `5e69dd9`): 3-tile channel grid (Push `Smartphone`, Email `Mail`, In-app locked with an `always` tag) — only push/email are stored; used by the broadcast composer and `NotifyBlock`.
- Service `broadcastAPI` (`services/api.js:1134` `9368b5e` / `5e69dd9`): `list({source})` `GET /api/notifications/broadcast?source=`, `activity({source,event,status,page})` `GET /api/notifications/activity`, `get(id)` `GET /broadcast/:id`, `create(data)` `POST /broadcast` (payload now includes `channels`) all `credentials:"include"` (Super Admin).
...
**May 31, 2026 (Bug fix — VerifyEmailPage double-invocation)** — React 18 StrictMode intentionally mounts → unmounts → remounts components in development, causing the `useEffect` in `VerifyEmailPage` to fire twice. The first call consumed the token (deleted from DB + set `email_verified = TRUE`); the second call found no token and returned `INVALID_TOKEN`, displaying the "Invalid link" error even though verification succeeded. Fixed by introducing a module-level `Set` (`verifyingTokens`) that persists across StrictMode remounts (unlike a `useRef`, which is reset on remount). The second invocation detects the token is already in the Set and skips the API call entirely. Has no effect in production where StrictMode double-invocation is disabled.
