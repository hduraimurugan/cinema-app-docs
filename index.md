...
| [backend.md](./backend.md)                                                             | Backend API reference                  | All endpoints, DB schema, auth (email + OAuth), middleware, payment integration, Sentry monitoring, Winston logging |
| [users.md](./users.md)                                                                 | User frontend documentation            | Pages, components, auth flow (email + Google OAuth), booking + payment flow                                                |
| [admin.md](./admin.md)                                                                 | Admin panel documentation              | Movie management, screen designer, show scheduling, bookings overview, OAuth (Google + GitHub)                               |
...
| [registration-and-onboarding-flow.md](./registration-and-onboarding-flow.md)           | Account lifecycle and tenancy model    | Register → verify → login → onboarding, team invites, platform vs org roles, table relationships, cascade rules, org/hall resolution |
...
| [backend_tests.md](./backend_tests.md)                                                                 | Test suite documentation               | 352 tests across 30 files, coverage report, test patterns, infrastructure overview                 |
...
*June 14, 2026 — Movie backdrop_path schema migration & UI integration: Idempotently added backdrop_path column to the movies database table; created GET `/api/movies/migrate-backdrops` endpoint for manual DB schema update and automated TMDB backdrop image backfill suited for Vercel deployments; updated CRUD controller operations and validated UUID parameter formatting; integrated Backdrop URL uploads/TMDB import in Movie Management forms and TMDB sync logic in Admin panel; updated user frontend HeroCarousel to display high-quality backdrop images with poster fallbacks.*
...
*June 21, 2026 — MoviesPage & MoviesList premium refinement: Added `hero-vignette` radial overlay for cinematic depth on hero slides. Book Now buttons upgraded to `custom-hover` class with scale animations (`hover:scale-[1.02]`, `hover:scale-105`). Autoplay progress bar keyframes added (commented out in HeroCarousel). Category tabs switched to `glass-effect p-1 rounded-xl shadow-sm` with `shadow-primary/20` active state and `bg-background/50` inactive hover. `ArrowRight` icon added to SectionHeader's "View All" button with hover translation animation. Scroll chevron buttons upgraded to `arrow-glass` class (premium glass with backdrop-blur, card/85 background, primary glow on hover). Scroll row wrapped with `scroll-fade-left`/`scroll-fade-right` fade edge overlays (48px gradient, `opacity: 0.85` when scrollable). MovieCard: added `card-glow-border` (primary border glow on hover), taller bottom gradient (`from-black/65 via-black/20 to-transparent h-28`), enhanced rating badge (`tabular-nums`, `shadow-lg`, moved to `top-3 left-3`), genre tag positioning adjusted, Book Now overlay now gradient-based. New CSS utilities: `hero-vignette`, `card-glow-border`, `scroll-fade-left/right`, `scroll-fade-visible`, `glow-soft`, `arrow-glass`, `animate-hero-progress`. Files: src/pages/MoviesPage.jsx, src/components/MoviesList.jsx, src/index.css.

*June 14, 2026 — Seating selection & SeatCountModal visual redesign: SeatCountModal upgraded with animated SVG vehicle graphics (Cycles, Vespa, Tuk-Tuk, Cars, SUVs, Buses with keyframe animations) shifting dynamically per seat count; custom 1-10 slider picker; dynamic seating categories pricing & availability breakdown from live seat layout data. SeatSelectionPage enhanced with 3D miniature seat buttons (cushion borders + scale-ups), a curved 3D cinema screen with ambient project beam glow, section divider pills with gradient borders, and a floating glassmorphic checkout dock. Refactored global button hover rules in `index.css` into distinct light/dark modes to prevent white-on-white text visibility collision for outline/ghost buttons in light mode. Excluded solid pink/red action buttons (checkout and modal select) using `custom-hover` to bypass global hover selectors and preserve design branding. Enabled closing/dismissing the SeatCountModal once a seat count has been set, and configured the toolbar seat count pill as an edit button.*
...
*June 22, 2026 — Multi-tenant Organization Onboarding, RBAC Gating, and Database Cleanup: Migrated database schema to be fully organization-centric; dropped legacy global `settings` table and transitioned convenience fee + GST configuration to `organization_settings`. Created a multi-step onboarding wizard in `OnboardingPage.jsx` (Organization Name → Cinema Hall Details → Location Map Picker) calling transactionally backed backend onboarding route `POST /api/auth/onboarding` to initialize tenants, roles, permissions, settings, and first halls. Applied strict role-based permission routing in frontend (`AdminProtectedRoutes.jsx`) with dynamic sidebar navigation hiding/showing options based on specific user permissions and platform `superAdminOnly` status (e.g. Ads, Customers, Hall Admins). Backend test suite updated to avoid database race conditions under parallel Vitest workers by resolving settings relative to the authenticated user's organization.*
