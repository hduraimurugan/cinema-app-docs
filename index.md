...
*March 12, 2026 — Shows Management date selector redesigned: added left/right chevron arrow buttons for week-by-week navigation. New `weekOffset` state shifts the visible 7-day window by ±7 days per click. Past dates are accessible. A week range label (e.g. "March 12 – 18, 2026") is shown above the pills. Pills remain fixed-width (`w-14`), left-aligned with consistent `gap-2` spacing. (`ShowsManagement.jsx`)*
