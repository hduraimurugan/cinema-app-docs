...
*June 14, 2026 — MoviesPage & MoviesList premium redesign: Full cinema-grade discovery overhaul. New HeroCarousel — full-width auto-rotating carousel (5s interval, hover pause, dot indicators, 400ms fade transitions) showcasing top 5 highest-rated now-showing movies with poster (object-contain), gradient overlays, featured badge, rating/duration/genre metadata, and dual CTAs (Book Now → showtimes, View Details → info). New category tabs — pill-style Now Showing / Coming Soon toggle with aria-pressed, keyed remount on switch. MovieCard premium rewrite — rounded-2xl, card-hover (translateY + scale + shadow-xl), always-visible rating badge (fill-rating text-rating via --rating token), always-visible genre tags on poster, bottom gradient for readability, hover overlay with Book Now CTA, role="button" + keyboard nav + focus-ring. AdBanner — yellow AD badge (bg-rating/80 text-rating-foreground) added, x-axis padding removed. Section headers — movie count badge, larger typography. States — shimmer loading, icon-driven error/empty. Design tokens — --rating/--rating-foreground (golden star), 9 new utility classes (hero-gradient-t/r, card-hover, shimmer, focus-ring, scroll-snap-x/start, featured-glow). Files: src/pages/MoviesPage.jsx, src/components/MoviesList.jsx, src/components/AdBanner.jsx, src/index.css.
