...
**Push via Firebase** (`src/lib/firebase.js:1` + `public/firebase-messaging-sw.js:1`, `2ab168b`): `firebase` `initializeApp` `VITE_FIREBASE_*`, `requestPushToken` (`isSupported`→`Notification.requestPermission`→`register("/firebase-messaging-sw.js")`→`getToken({vapidKey: VITE_FIREBASE_VAPID_KEY})`), `onForegroundMessage`; SW `importScripts 10.14.1/firebase-app-compat.js` `cinemax-644cf` with `onBackgroundMessage` `CineMax` icon `cinemax_logo.png` (identical to admin `35908f4` but `title||'CineMax'`).
